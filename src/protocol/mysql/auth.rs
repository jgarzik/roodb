...
impl HandshakeResponse41 {
    ...
    pub fn parse(data: &[u8]) -> ProtocolResult<Self> {
        ...
        // Auth response
        let auth_response = if capability_flags & capabilities::CLIENT_PLUGIN_AUTH_LENENC_CLIENT_DATA != 0 {
        ...
        let database = if capability_flags & capabilities::CLIENT_CONNECT_WITH_DB != 0 && offset < data.len() {
        ...
        // Auth plugin name (if CLIENT_PLUGIN_AUTH)
        let auth_plugin_name = if capability_flags & capabilities::CLIENT_PLUGIN_AUTH != 0 && offset < data.len() {
...
mod tests {
    ...
    fn test_verify_password() {
        ...
        assert!(verify_mysql_native_password(scramble, password, &auth_response));

        // Wrong password should fail
        assert!(!verify_mysql_native_password(scramble, "wrong", &auth_response));

        // Corrupted response should fail
        let mut bad_response = auth_response.clone();
        bad_response[0] ^= 0xff;
        assert!(!verify_mysql_native_password(scramble, password, &bad_response));
    }
